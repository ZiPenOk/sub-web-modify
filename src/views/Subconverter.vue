...
export default {
  data() {
    return {
      ...
      form: {
        sourceSubUrl: "",
        clientType: "",
        customBackend: this.getUrlParam() == "" ? "https://api.sh1nyan.fun" : this.getUrlParam(),
        shortType: "https://v1.mk/short",
        remoteConfig: "https://github.com/ZiPenOk/Custom_OpenClash_Rules/raw/dev/cfg/Custom_Clash.ini",
        excludeRemarks: "",
        includeRemarks: "",
        filename: "",
        rename: "",
  ...
  methods: {
    ...
    tanchuang() {
      this.$alert(`<div style="text-align:center;font-size:15px"><strong><span style="font-size:20px;color:red">自用前端面板, 小心我偷你订阅喔!🤣</span></strong></br></div>`, '信息面板', {
    ...
    getBackendVersion() {
      this.$axios
          ...
          .then(res => {
            ...
            let c = this.form.customBackend.includes("api.sh1nyan.fun");
            a ? this.$message.success(`${this.backendVersion}` + "肥羊负载均衡增强版后端，已屏蔽免费节点池（会返回403），额外支持vless reality+hysteria+hysteria2订阅转换") : b ? this.$message.success(`${this.backendVersion}` + "本地局域网自建版后端") : c ? this.$message.success(`${this.backendVersion}` + "自用后端 全兼容，支持 vless reality+hysteria+hysteria2 clash and singbox") : this.$message.success(`${this.backendVersion}` + "官方原版后端不支持vless/hysteria订阅转换");
          })
          .catch(() => {
